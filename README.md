...
`ncap-evaluate` checks one model and `ncap-recovery` tests recovery after damage. `ncap-horizons` compares short and long training rollouts using `configs/horizon-study.json`. `ncap-bounds` tests optional state limits using `configs/bound-study.json`. Run any command with `--help` for options.
